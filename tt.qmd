...
library(ggplot2)
library(dplyr)
library(scales)  # pra formatar valores
...
# 1. Pacotes ESSENCIAIS (sem dependências extras)
library(WDI)
library(ggplot2)

# 2. Dados garantidos (sem erros)
dados <- WDI(
...
# 3. Processamento à prova de falhas
...
# 4. Gráfico robusto (formato básico garantido)
ggplot(dados, aes(x = year, y = SP.DYN.LE00.IN, color = country)) +
  ...
  labs(
    title = NULL, # Removendo título para controle manual no Quarto
    x = "Ano",
    y = "Expectativa de Vida (anos)",
    color = "País"
  ) +
